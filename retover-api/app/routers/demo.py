...
def _run_realtime_turn(
    ...
    try:
        with sessionmaker() as db:
            ...
            context = AgentRuntimeContext(org_id=org_id, user_id=admin_id, agent_id=agent.id, workflow_type="browser-demo-realtime")
            ...
            selected_model = requested_model if requested_model in REALTIME_GROQ_MODEL_OPTIONS else REALTIME_GROQ_MODEL
            cartesia_config = CartesiaProviderService._config(db)
            ...
            if not assistant_text.strip():
                assistant_text = _retry_empty_llm_once(db, provider="groq", system_prompt=system_prompt, messages=messages, model=selected_model, tools=tools)
