...
def _telnyx_config(db: Session) -> dict[str, Any]:
    ...
    return {
        **config,
        ...
        "fallback_caller_id": str(config.get("fallback_caller_id") or config.get("default_outbound_number") or "").strip(),
...
class TelnyxVoiceAdapter:
    ...
    def _create_call(
        *,
        ...
        if client_state:
            payload["client_state"] = json.dumps(client_state, separators=(",", ":"))
        with httpx.Client(timeout=20.0) as client:
...
class TelnyxExecutionService:
    ...
    def start_call(
        ...
        *,
        ...
        caller_id = verified_caller_id or config.get("fallback_caller_id") or config.get("default_outbound_number")
        if not caller_id:
        ...
        if log is None:
            if not org_id:
                ...
                if isinstance(client_state, str):
                    try:
                        org_id = json.loads(client_state).get("org_id")
                    except Exception:
                        org_id = None
            if not org_id:
                return None
