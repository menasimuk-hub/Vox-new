...
function applyTemplateDraft(setDraft, tpl) {
  ...
  setDraft({
    ...
    active_for_appointment: tpl?.active_for_appointment !== false,
...
export default function WaAppointmentTemplateModal({ templateId, open, onClose, onSaved }) {
  ...
  const varLabels = appointmentVarLabels(template?.sales_template_key)
  ...
  const applySuggestedRename = async () => {
    ...
    clearSyncError()
    try {
      const data = await apiFetch(`/admin/wa-appointment/templates/${templateId}/rename-for-sync`, {
        method: 'POST',
  ...
  const applySuggestedLanguage = async () => {
    ...
    try {
      ...
      const data = await apiFetch(`/admin/wa-appointment/templates/${templateId}`, {
        method: 'PUT',
        body: JSON.stringify({
          display_name: draft.display_name,
          category: draft.category,
          language: syncFix.suggestedLanguage,
          active_for_appointment: draft.active_for_appointment,
          components,
  ...
  const loadTemplate = useCallback(async () => {
    ...
    try {
      ...
      setIsDirty(false)
      const previewData = await apiFetch(`/admin/wa-appointment/templates/${templateId}/preview`)
  ...
  const pushToTelnyx = async () => {
    ...
    try {
      ...
      const data = await apiFetch(`/admin/wa-appointment/templates/${templateId}/push`, { method: 'POST', body: '{}' })
      setTemplate(data?.template || template)
      applyTemplateDraft(setDraft, data?.template || template)
      setIsDirty(false)
  ...
  const toggleHidden = async () => {
    ...
    setWorking('hide')
    try {
      const data = await apiFetch(`/admin/wa-appointment/templates/${templateId}/set-active`, {
        method: 'POST',
        body: JSON.stringify({ active_for_appointment: !draft.active_for_appointment }),
      })
      setTemplate(data?.template || template)
      applyTemplateDraft(setDraft, data?.template || template)
    ...
    <div className="modal-overlay" role="dialog" aria-modal="true">
      <div className="modal-card wa-template-modal">
        <div className="modal-header">
          <div>
            <h2>{draft?.display_name || template?.display_name || 'Appointment template'}</h2>
        ...
        <div className="modal-footer">
          ...
          <button type="button" className="btn btn-outline" disabled={working} onClick={() => void toggleHidden()}>
            {draft?.active_for_appointment ? 'Hide' : 'Show'}
          </button>
