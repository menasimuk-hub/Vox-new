...
export default function TicketTable({ tickets = [], onOpen, onBulkStatus, onBulkAssign, onArchive, hideArchiveAction = false, admins = [], cannedReplies = [], loading }) {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = tickets.filter((t) => (status === 'all' || t.status === status) && (priority === 'all' || (t.priority || 'normal') === priority) && (channel === 'all' || (t.channel || 'email') === channel) && (!q || `${t.requester_email || ''} ${t.created_by_email || ''} ${t.subject} ${t.public_ref} ${t.organisation_name || ''}`.toLowerCase().includes(q)))
