...
export default function SmsTemplateEdit() {
  ...
  const charCount = String(draft.body || '').length
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const row = await apiFetch(`/admin/messaging/sms/templates/${encodeURIComponent(templateKey)}`)
      setDraft({
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        const created = await apiFetch('/admin/messaging/sms/templates', {
          method: 'POST',
          body: JSON.stringify({ ...payload, template_key: key }),
        })
        navigate(`/settings/email/sms/${encodeURIComponent(created.template_key)}/edit`, { replace: true })
        return
      }
      await apiFetch(`/admin/messaging/sms/templates/${encodeURIComponent(templateKey)}`, {
        method: 'PUT',
      ...
      <div className="pageTop">
        <div>
          <div className="muted" style={{ fontSize: 12, marginBottom: 6 }}>
            <Link to="/settings/email?tab=sms" style={{ color: 'var(--grn)' }}>
              ← Back to SMS templates
            </Link>
          </div>
          <h1>{isNew ? 'Create SMS template' : `Edit · ${draft.name || templateKey}`}</h1>
          ...
          <div className="card msgTemplateEditor">
            ...
            <div className="cardBody">
              ...
              <div className="msgFieldBlock">
                ...
                <textarea
                  className="input msgFieldSms"
                  value={draft.body}
                  onChange={(e) => setDraft((d) => ({ ...d, body: e.target.value }))}
                  placeholder="Your verification code: {{code}}"
                />
                <p className="fieldHint">
                  {charCount} / {SMS_MAX} characters
                  {charCount > SMS_MAX ? ' — may split into multiple SMS segments' : ''}
                </p>
              </div>

              <div className="msgFieldBlock">
                <label className="label">Preview</label>
                <div className="smsPreviewFull">
                  <p>{draft.body || 'Message preview appears here.'}</p>
                </div>
              </div>
              ...
              <div className="note" style={{ marginTop: 14, marginBottom: 0 }}>
                SMS via Telnyx —{' '}
                <a href="/integrations/telnyx" style={{ color: 'var(--grn)' }}>
                  Integrations → Telnyx
                </a>
                . Reference template by key: <code>{draft.template_key || '(set on save)'}</code>
              </div>
            </div>
          </div>
        )}
