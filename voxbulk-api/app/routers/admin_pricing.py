...
def _pricing_db_error(exc: Exception) -> HTTPException:
    ...
    if (
        ...
        return HTTPException(
            ...
            detail="Pricing data is still syncing. Wait a moment and click Retry.",
        )
...
def _run_pricing_db(db: Session, fn: Callable[[], T]) -> T:
    ...
    for attempt in range(3):
        try:
            ...
            db.rollback()
            last_exc = exc
            logger.warning("pricing_admin_db_retry attempt=%s error=%s", attempt + 1, exc)
            ensure_pricing_schema()
    assert last_exc is not None
    raise _pricing_db_error(last_exc) from last_exc
