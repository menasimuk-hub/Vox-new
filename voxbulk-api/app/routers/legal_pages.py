...
def public_legal_page(slug: str, db: Session = Depends(get_db)):
    row = LegalPageService.get_public_page(db, slug)
    if row is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Legal page not found")
    data = legal_page_to_dict(row)
...
@router.put("/admin/legal-pages/{slug}", response_model=LegalPageOut)
