"""WA Templates → AI Demo — WhatsApp template library."""
...
from app.data.demo_whatsapp_template_catalog import (
    DEMO_WA_TEMPLATE_KEYS,
    DEMO_WA_TEMPLATE_SPECS,
    demo_catalog_telnyx_names,
    demo_spec_by_key,
    demo_spec_components,
)
...
class DemoWhatsappTemplateError(RuntimeError):
    def __init__(self, message: str, *, payload: dict[str, Any] | None = None):
        super().__init__(message)
        self.payload = payload


def demo_template_to_dict(row: TelnyxWhatsappTemplate) -> dict[str, Any]:
    ...
    spec = demo_spec_by_key(str(row.sales_template_key or ""))
    ...
    return {
        **base,
        ...
        # Reuse active_for_interview column (no separate demo flag on the model).
        "active_for_demo": bool(getattr(row, "active_for_interview", True)),
...
class DemoWhatsappTemplateService:
    @staticmethod
    def _is_demo_row(row: TelnyxWhatsappTemplate) -> bool:
        key = str(row.sales_template_key or "").strip().lower()
        if key in DEMO_WA_TEMPLATE_KEYS:
            return True
        name = str(row.name or "").strip().lower()
        return name in demo_catalog_telnyx_names()

    @staticmethod
    def get_template(db: Session, template_id: int) -> TelnyxWhatsappTemplate | None:
        row = db.get(TelnyxWhatsappTemplate, template_id)
        if row is None or not DemoWhatsappTemplateService._is_demo_row(row):
            return None
        return row
    ...
    def ensure_catalog_seeded(db: Session) -> list[TelnyxWhatsappTemplate]:
        ...
        for spec in DEMO_WA_TEMPLATE_SPECS:
            ...
            existing = DemoWhatsappTemplateService._find_row_for_spec(db, key, telnyx_name)
            components = demo_spec_components(spec)
    ...
    def list_templates(db: Session) -> list[dict[str, Any]]:
        DemoWhatsappTemplateService.ensure_catalog_seeded(db)
        payload: list[dict[str, Any]] = []
        for key in DEMO_WA_TEMPLATE_KEYS:
            spec = demo_spec_by_key(key)
            if not spec:
                continue
            row = DemoWhatsappTemplateService._find_row_for_spec(
                db,
                key,
            ...
            if row is None:
                continue
            payload.append(demo_template_to_dict(row))
        return payload
    ...
    def get_template_detail(db: Session, template_id: int) -> dict[str, Any] | None:
        ...
        if row is None:
            return None
        return demo_template_to_dict(row)
    ...
    def save_draft(db: Session, row: TelnyxWhatsappTemplate, payload: dict[str, Any]) -> TelnyxWhatsappTemplate:
        ...
        if "language" in payload and str(payload.get("language") or "").strip():
            ...
            if lang_error:
                raise DemoWhatsappTemplateError(
                    lang_error,
        ...
        if "active_for_demo" in payload or "active_for_interview" in payload:
            flag = payload["active_for_demo"] if "active_for_demo" in payload else payload["active_for_interview"]
            row.active_for_interview = bool(flag)
    ...
    def rename_for_meta_sync(db: Session, row: TelnyxWhatsappTemplate, new_name: str) -> TelnyxWhatsappTemplate:
        try:
            ...
            raise DemoWhatsappTemplateError(str(exc), payload=exc.payload) from exc
    ...
    def sync_from_telnyx(db: Session) -> dict[str, Any]:
        logger.info("demo_wa_template_sync_start")
        try:
            ...
            for key in DEMO_WA_TEMPLATE_KEYS:
                ...
                if result is not None:
                    linked += 1
            demo_names = demo_catalog_telnyx_names()
            matched = list(
                db.execute(
                    select(TelnyxWhatsappTemplate).where(
                        TelnyxWhatsappTemplate.sales_template_key.in_(list(DEMO_WA_TEMPLATE_KEYS))
                    )
                ).scalars().all()
            )
            return {
                **summary,
                "ok": True,
                "message": f"Synced {summary.get('synced', 0)} Telnyx templates; {len(matched)} demo templates in library."
                + (f" Linked {linked} existing remote template(s)." if linked else ""),
                "demo_templates": len(matched),
                "demo_linked": linked,
            ...
            logger.exception("demo_wa_template_sync_failed")
            return {
                "ok": False,
                "message": f"WA Demo sync failed: {exc}",
                "errors": [str(exc)],
            }
