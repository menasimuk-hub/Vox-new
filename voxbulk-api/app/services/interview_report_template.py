...
def _call_outcome_section(call_outcome: dict[str, Any] | None, *, pdf: bool = False) -> str:
    data = call_outcome or {}
    if not data:
        return ""
    email = data.get("missed_call_email") or {}
    rows = [
        ("Call outcome", str(data.get("status_label") or data.get("status") or "—")),
        ("Voicemail policy", str(data.get("voicemail_behavior_label") or "—")),
        ("Follow-up email", str(email.get("outcome_label") or "—")),
    ]
    if email.get("template_key"):
        rows.append(("Email template", str(email.get("template_key"))))
    if email.get("to"):
        rows.append(("Sent to", str(email.get("to"))))
    notes = str(data.get("experience_notes") or "").strip()
    if notes:
        rows.append(("Experience notes", notes))
    body = "".join(
        f"""<div class="criteria-row">
        <div class="criteria-head"><strong>{_e(label)}</strong></div>
        <div class="criteria-note">{_e(value)}</div>
        </div>"""
        for label, value in rows
    )
    if pdf:
        return f'<div class="section"><div class="section-title">Call outcome &amp; follow-up</div>{body}</div>'
    return f"""<div class="section">
      {_section_header("Call Outcome & Follow-up", ICON_CALL, "amber")}
      {body}
    </div>
