...
def _fill_en_human_behavior(code: str) -> None:
    ...
    p["human_behavior"] = (
        ...
        "- If they ask about the email/link after a reschedule goodbye: confirm once "
        "(\"Yes — you'll get the reschedule link by email shortly\") then say goodbye and Hangup.\n"
...
def _fill_ar_human_behavior(code: str) -> None:
    ...
    p["human_behavior"] = (
        ...
        "قواعد إنهاء المكالمة: متقفّلش الخط وهو بيسأل. لو سأل عن الإيميل/اللينك بعد إعادة الجدولة، أكّد مرة "
        "(«أيوه هتوصّلك لينك إعادة الجدولة على الإيميل») بعدين مع السلامة ثم Hangup.\n"
