...
def _canonical_flow_ar() -> str:
    return (
        ...
        "بعد مع السلامة: استخدم أداة Hangup (أو end_call) فورًا. متسيبش الخط مفتوح. متقولش «انتهت المكالمة» — مع السلامة وبعدين Hangup.\n"
...
def _canonical_flow_en() -> str:
    return (
        ...
        "After goodbye: immediately use the Hangup tool (or end_call). Do not stay on the line. "
...
def _fill_en_human_behavior(code: str) -> None:
    ...
    p["human_behavior"] = (
        ...
        "When the conversation is finished (reschedule goodbye, recording decline, or final closing): say goodbye once, "
        "then immediately invoke the Hangup tool (or end_call). Never leave silence on an open line. "
        "Never say the words 'call ended'. Goodbye, then Hangup — no extra talk.\n"
        "After recording consent is given, call mark_recording_consent. "
        "After each screening question you ask, call mark_question_asked with the question number.\n"
        f"{p['listening']}\n"
...
def _fill_ar_human_behavior(code: str) -> None:
    ...
    p["human_behavior"] = (
        ...
        "بعد الموافقة على التسجيل: نادِ mark_recording_consent. بعد كل سؤال فرز: نادِ mark_question_asked برقم السؤال.\n"
        "لما تخلّص (إعادة جدولة / رفض تسجيل / إغلاق): قل مع السلامة مرة، بعدين Hangup / end_call فورًا. "
