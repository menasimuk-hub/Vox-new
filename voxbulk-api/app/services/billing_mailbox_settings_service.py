"""Billing mailbox settings (IMAP receive + outbound From billing@voxbulk.com)."""
...
from app.models.billing_mailbox_settings import BILLING_MAILBOX_ROW_ID, BillingMailboxSettings


class BillingMailboxSettingsService:
    @staticmethod
    def get_row(db: Session) -> BillingMailboxSettings:
        obj = db.execute(
            select(BillingMailboxSettings).where(BillingMailboxSettings.id == BILLING_MAILBOX_ROW_ID)
        ).scalar_one_or_none()
        if obj is None:
            obj = BillingMailboxSettings(id=BILLING_MAILBOX_ROW_ID)
            db.add(obj)
            db.commit()
            db.refresh(obj)
        return obj
    ...
    @staticmethod
    def compute_status(row: BillingMailboxSettings) -> tuple[bool, list[str]]:
        missing: list[str] = []
    ...
    @staticmethod
    def to_public_dict(db: Session, row: BillingMailboxSettings) -> dict[str, Any]:
        configured, missing = BillingMailboxSettingsService.compute_status(row)
        return {
            "mailbox_email": row.mailbox_email or "billing@voxbulk.com",
    ...
    def upsert(
        ...
        *,
        ...
        is_enabled: bool,
        password: str | None,
    ) -> BillingMailboxSettings:
        row = BillingMailboxSettingsService.get_row(db)
        row.mailbox_email = (mailbox_email or "billing@voxbulk.com").strip().lower()
        ...
        row.sync_interval_minutes = max(15, min(int(sync_interval_minutes or 60), 1440))
