...
class ZoomService:
    ...
    @staticmethod
    def _telnyx_has_any_zoom_data(telnyx_cfg: dict[str, Any]) -> bool:
    ...
    def _config(db: Session) -> dict[str, Any]:
        """Resolve Zoom OAuth credentials from the Telnyx provider row (single source of truth)."""
        ...
        base_url = str(telnyx_cfg.get("zoom_base_url") or "").strip().rstrip("/")
        default_base = "https://api.zoom.us/v2"

        if not ZoomService._telnyx_has_any_zoom_data(telnyx_cfg):
    ...
    def is_interview_delivery_enabled(db: Session) -> bool:
        """True when Telnyx → Zoom OAuth credentials are stored and complete."""
