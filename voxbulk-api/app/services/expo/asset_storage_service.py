"""Store Expo booth PDF/image uploads under data/expo-assets/{org_id}/."""
...
def ensure_expo_assets_root() -> Path:
    EXPO_ASSETS_ROOT.mkdir(parents=True, exist_ok=True)
    return EXPO_ASSETS_ROOT
...
def resolve_storage_abs_path(storage_path: str | None) -> Path | None:
    ...
    try:
        abs_path.relative_to(EXPO_ASSETS_ROOT.resolve())
    except ValueError:
        return None
    return abs_path if abs_path.is_file() else None


async def save_expo_asset_upload(*, org_id: str, upload: UploadFile) -> dict:
    ...
    if content_type and content_type not in ALLOWED_CONTENT_TYPES and not content_type.startswith("image/"):
        # Allow empty/unknown from some browsers; reject clearly wrong types
    ...
    if len(raw) > MAX_EXPO_ASSET_BYTES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="File too large (max 20 MB).",
        )

    ensure_expo_assets_root()
    org_dir = EXPO_ASSETS_ROOT / oid
