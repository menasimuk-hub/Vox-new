"""Expo mailbox settings (outbound From for Expo visitor / exhibitor emails)."""
...
from app.models.expo_mailbox_settings import EXPO_MAILBOX_ROW_ID, ExpoMailboxSettings

DEFAULT_MAILBOX = "expo@voxbulk.com"
DEFAULT_FROM_NAME = "VOXBULK Expo"


class ExpoMailboxSettingsService:
    @staticmethod
    def get_row(db: Session) -> ExpoMailboxSettings:
        obj = db.execute(
            select(ExpoMailboxSettings).where(ExpoMailboxSettings.id == EXPO_MAILBOX_ROW_ID)
        ).scalar_one_or_none()
        if obj is None:
            obj = ExpoMailboxSettings(
                id=EXPO_MAILBOX_ROW_ID,
