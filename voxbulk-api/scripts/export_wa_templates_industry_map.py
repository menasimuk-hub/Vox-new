...
COLUMNS = ["template_name","product_line","industry_slug","industry_name","survey_type_slug","survey_type_name","template_key","name_variant","db_found","telnyx_status","display_name","body_preview","source"]
...
def write_xlsx(rows: list[dict], output: Path) -> None:
    ...
    wb.save(output)
def main() -> None:
    parser = argparse.ArgumentParser(description="Export WA template industry map")
    parser.add_argument("--names-file", type=Path, default=DEFAULT_NAMES)
