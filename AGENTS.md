...
- Production deploys from a Linux VPS via `./deploy-vps.sh` (or `./vox.sh update`) — never from Windows. Dual gunicorn (`:8000` + `:8001`) rolling reload; public/admin/dashboard are static wwwroot. See the `vps-deploy` rule.
