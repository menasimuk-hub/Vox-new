...
export function SocialAuthButtons({ onOAuth, oauthLoading, compact }: SocialAuthButtonsProps) {
  ...
  if (readyProviders.size === 0) return null;
...
function SocialBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={loading}
      className={`flex items-center justify-center gap-2.5 ${compact ? "h-11" : "h-12"} rounded-xl border border-border bg-white hover:bg-secondary/60 text-[13.5px] font-medium text-foreground transition-colors disabled:opacity-60`}
    >
