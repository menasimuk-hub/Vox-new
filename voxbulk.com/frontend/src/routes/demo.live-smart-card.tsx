...
export const Route = createFileRoute("/demo/live-smart-card")({
  validateSearch: (search: Record<string, unknown>) => ({
    session: typeof search.session === "string" ? search.session : "",
    service: typeof search.service === "string" ? search.service : "smart_card",
  }),
  component: LiveSmartCardPage,
});

function LiveSmartCardPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLiveDemoResponse({
        session_id: session,
        service: service || "smart_card",
        name,
        company,
        score: 5,
        comment: "Smart Card scan from live demo",
      });
      setDone(true);
      toast.success("Lead attributed — check the demo dashboard");
    } catch (err) {
    ...
    <div className="bg-background text-body antialiased min-h-screen flex flex-col">
      ...
      <main className="flex-1 pt-[110px] pb-16 px-5">
        <div className="max-w-md mx-auto rounded-3xl border border-border bg-white p-6 shadow-elegant">
          <h1 className="text-[22px] font-bold text-heading">Save contact</h1>
          <p className="text-[14px] text-body mt-2">Smart Card demo QR — lands on the live dashboard.</p>
          {done ? (
            <p className="mt-6 text-emerald-700 font-semibold">Saved — look at the demo screen.</p>
          ) : (
